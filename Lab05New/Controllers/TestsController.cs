...
namespace Lab05New.Controllers
{
    ...
    public class TestsController : Controller
    {
        ...
        /*[Authorize]
        [HttpPost]
         public async Task<IActionResult> Index(string name = "", int difficulty = 0, int topic = 0, int page = 1)
        {
            // проверить, было ли отправление формы, тогда записывать в куки, а если нет, то брать из куки
            ...
            var viewModel = GetTestsViewModel(name, difficulty, topic, page).Result;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                if(!Request.Cookies.TryGetValue("testName", out name))
                {
                    name = "";
                }
            }
        ...
        public async Task<IActionResult> Take(int id, int questionNumber = 1)
        {
            ...
            Question question = test.Questions.Skip((questionNumber - 1)).First();

            QuestionsViewModel viewModel = new QuestionsViewModel(question, new PageViewModel(count, questionNumber, 1)); */
        ...
        public async Task<IActionResult> Take(int testId, string[] answers)
        {
            ...
            if (currentResult == null)
            {
                _db.Results.Add(new Result
                {
                    ...
                    TestId = _currentTest.TestId,
                    Mark = mark // поменять на от 1 до 10
            ...
            // db.savechanges();
            // добавление в completionmarks
            // генерация представления для отображения результата
        ...
        public async Task<IActionResult> Create(CreateTestViewModel model)
        {
            ...
            {
                // try catch на _db.tests.add, в catch (создать метод и впихнуть туда то, что сейчас в else
                isValid = true;
        ...
        public async Task<IActionResult> Edit(EditTestViewModel model)
        {
            ...
            /*var diffs = _db.Difficulties.ToList();
            var topics = _db.Topics.ToList();

            var a = model.Difficulties;
            var b = model.Topics;*/
